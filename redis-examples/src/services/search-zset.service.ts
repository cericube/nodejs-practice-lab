// src/services/search-zset.service.ts
...
export class SearchZSetservice {
  ...
  async increaseSearchKeywordScore(keyword: string, score = 1): Promise<number> {
    ...
    // 검색어이 없으면 추가하고, 있으면 점수를 누적한 뒤 최종 점수를 반환합니다.
  ...
  async getPopularKeywords(limit = 10): Promise<PopularKeywordOutput[]> {
    ...
    // 인기 검색어 랭킹에서 상위 검색어을 점수와 함께 조회합니다.
    // REV 옵션으로 점수가 높은 순서의 지정 범위를 반환하며, 검색어이 없으면 빈 배열을 반환합니다.
  ...
  async getKeywordRank(keyword: string): Promise<number | null> {
    ...
    // 0부터 시작하는 순위를 반환하며, 검색어이 랭킹에 없으면 null을 반환합니다.
  ...
  async removeKeyword(keyword: string): Promise<void> {
    ...
    // 인기 검색어 랭킹에서 지정한 검색어을 제거합니다.
    // 검색어과 점수를 함께 제거하고 제거한 수를 반환하며, 검색어이 없으면 0을 반환합니다.
