...
function parseProductStockHash(hash: Record<string, string>): ProductStockOutput | null {
  ...
  return {
    productId: Number(hash.productId),
    name: hash.name,
    stock,
    reservedStock,
    availableStock: Number(hash.availableStock),
    status: hash.status,
...
export class ProductHashService {
   ...
   * 3. reservedStock 입력값을 기준으로 ProductStockOutput 형태를 만듭니다.
  ...
  async updateProductStock(
    ...
    // reservedStock은 DB 컬럼이 아니므로 입력값이 없으면 0으로 계산합니다.
    const reservedStock = input.reservedStock ?? 0;
