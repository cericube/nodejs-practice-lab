// src/services/user-zset.service.ts
...
export class UserZSetServicve {
  ...
  async setUserPointRankingScore(userId: number, point: number): Promise<void> {
    ...
    // 사용자 포인트 랭킹에 사용자과 점수를 저장합니다.
    // 새 사용자을 추가하거나 기존 점수를 갱신하고 새로 추가된 사용자 수를 반환합니다.
  ...
  async getTopUserPointRanking(limit = 10): Promise<UserRankingOutput[]> {
    ...
    // 사용자 포인트 랭킹에서 상위 사용자을 점수와 함께 조회합니다.
    // REV 옵션으로 점수가 높은 순서의 지정 범위를 반환하며, 사용자이 없으면 빈 배열을 반환합니다.
  ...
  async getUserPointRank(userId: number): Promise<number | null> {
    ...
    // 0부터 시작하는 순위를 반환하며, 사용자이 랭킹에 없으면 null을 반환합니다.
   ...
   * 2. 기존 Redis 랭킹을 삭제합니다.
   * 3. DB point 기준으로 Redis Sorted Set을 다시 구성합니다.
   *
   * 실습 포인트:
  ...
  async removeUserFromPointRanking(userId: number): Promise<void> {
    ...
    // 사용자 포인트 랭킹에서 지정한 사용자을 제거합니다.
    // 사용자과 점수를 함께 제거하고 제거한 수를 반환하며, 사용자이 없으면 0을 반환합니다.
