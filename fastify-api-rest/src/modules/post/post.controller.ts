...
export class PostController {
  ...
  updatePost(ids: PostIdParamsDto, input: PostUpdateBodyDto): Promise<PostUpdateResponseDto> {
  ...
  getPost(input: PostQueryDto): Promise<PostResponseDto> {
    return this.postService.getPost(input);
  }

  /** 게시글 목록을 필터링하여 조회 (GET list 대응) */
  listPosts(input: PostListQueryDto): Promise<PostListResponseDto> {
    return this.postService.listPosts(input);
  }
}
